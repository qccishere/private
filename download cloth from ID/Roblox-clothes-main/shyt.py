...
def get_csrf_token(cookie, proxies):
    """Fetches a CSRF token from Roblox."""
    if not cookie:
        return None
...
def get_asset_name(cookie, csrf_token, clothing_id, proxies):
    """Fetches the asset's name using the Roblox API."""
...
def get_asset_id(cookie, clothing_id, proxies):
    response = make_request(
        "get",
    ...
    if location:
        asset_id_response = make_request("get", location, timeout=10, proxies=proxies)
...
def get_png_url(cookie, asset_id, proxies):
    response = make_request(
        "get",
    ...
    if png_url:
        png_response = make_request("get", png_url, timeout=10, proxies=proxies)
        if png_response:
...
def download_clothing_image(cookie, clothing_id, proxies, asset_type="shirts"):
    try:
        ...
        csrf_token = get_csrf_token(cookie, proxies)
        if csrf_token:
            item_name = get_asset_name(cookie, csrf_token, clothing_id, proxies)
        else:
        ...
        asset_id = get_asset_id(cookie, clothing_id, proxies)
        ...
        png_content = get_png_url(cookie, asset_id, proxies)
...
def main():
    ...
    parser.add_argument('--threads', type=int, default=10, help='Number of concurrent threads for batch downloading (default: 10).')
